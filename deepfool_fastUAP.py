...
def deepfool_fastUAP(image, v, f, grads, num_classes=10, overshoot=0.02, max_iter=50):
    ...
    while k_i == label and loop_i < max_iter:
        ...
        for k in range(1, num_classes):
            ...
            r_i_k =  pert_k * w_k / np.linalg.norm(w_k.flatten())
            ...
            if np.all(v == 0):
                ...
                # determine which w_k to choose
                cos_k = np.dot(r_i_k.flatten(), v.flatten()) / (np.linalg.norm(r_i_k.flatten()) * np.linalg.norm(v.flatten())) 
                if cos_k > cos:
                    cos = cos_k
                    r_i = r_i_k
